...
const MarkdownRenderer = ({ content }: { content: string }) => {
  return (
    <ReactMarkdown
      // CRITICAL: remarkGfm is required to parse the "|" table syntax
      ...
      components={{
        ...
        ul: ({ children }) => (
          <ul className="list-disc pl-5 my-2 space-y-1 marker:text-zinc-400">
            {children}
          </ul>
        ),
        ...
        th: ({ children }) => (
          <th className="px-4 py-2 font-semibold text-zinc-900 dark:text-zinc-100 border-r last:border-r-0 border-zinc-200 dark:border-zinc-800">
            {children}
          </th>
        ),
        td: ({ children }) => (
          <td className="px-4 py-2 text-zinc-700 dark:text-zinc-300 border-r last:border-r-0 border-b last:border-b-0 border-zinc-200 dark:border-zinc-800">
            {children}
          </td>
        ),
        ...
        code: ({ inline, className, children, ...props }: any) => {
          ...
          return (
            <code
              className="bg-zinc-100 dark:bg-zinc-800 rounded px-1 py-0.5 text-sm font-mono text-zinc-800 dark:text-zinc-200"
              {...props}
            >
              {children}
            </code>
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setChatMessages([
        {
          role: "assistant",
          text: `Hello! I've analyzed **${file.name}**. What specific questions do you have about it?`,
        },
      ]);
    } catch (err: any) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setChatMessages((prev) => [
        ...
        {
          role: "assistant",
          text: "⚠️ Connection error. Please try asking again.",
        },
      ]);
    } finally {
      setIsChatting(false);
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 max-w-xl mx-auto text-lg">
            Upload any research paper. Our agent will digest the PDF, extract
            the core methodology, and let you chat directly with the text.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pt-8">
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/50 rounded-xl p-6 flex gap-4 items-start shadow-sm">
                ...
                <div>
                  <h4 className="text-sm font-bold text-indigo-800 dark:text-indigo-300 uppercase tracking-wider mb-1">
                    TL;DR
                  </h4>
                  <div className="text-lg font-medium text-indigo-950 dark:text-indigo-100 leading-relaxed">
                    <MarkdownRenderer
                      content={
                        result.summary.tl_dr ||
                        "Generating TL;DR failed. Please try again."
                      }
                    />
                  </div>
                </div>
                ...
                <Card className="bg-red-50/40 dark:bg-red-950/20 border-red-100 dark:border-red-900/30 shadow-sm">
                  <CardContent className="p-5 space-y-3">
                    ...
                    <div className="text-sm text-zinc-700 dark:text-zinc-300 leading-relaxed">
                      <MarkdownRenderer
                        content={
                          result.summary.core_problem ||
                          "No core problem identified."
                        }
                      />
                    </div>
                  </CardContent>
                ...
                <Card className="bg-emerald-50/40 dark:bg-emerald-950/20 border-emerald-100 dark:border-emerald-900/30 shadow-sm">
                  <CardContent className="p-5 space-y-3">
                    ...
                    <div className="text-sm text-zinc-700 dark:text-zinc-300 leading-relaxed">
                      <MarkdownRenderer
                        content={
                          result.summary.practical_implications ||
                          "No implications detailed."
                        }
                      />
                    </div>
                  </CardContent>
                ...
                <Card className="shadow-sm border-zinc-200/50 dark:border-zinc-800/50">
                  ...
                  <CardContent>
                    <ul className="space-y-2 text-sm text-zinc-600 dark:text-zinc-400 list-disc pl-4 marker:text-emerald-400">
                      {(
                        result.summary.strengths || ["No strengths noted."]
                      ).map((item, i) => (
                        <li key={i}>
                          <MarkdownRenderer content={item} />
                        </li>
                ...
                <Card className="shadow-sm border-zinc-200/50 dark:border-zinc-800/50">
                  ...
                  <CardContent>
                    <ul className="space-y-2 text-sm text-zinc-600 dark:text-zinc-400 list-disc pl-4 marker:text-amber-500">
                      {(
                        result.summary.limitations || ["No limitations noted."]
                      ).map((item, i) => (
                        <li key={i}>
                          <MarkdownRenderer content={item} />
                        </li>
                        ...
                        <Accordion type="single" collapsible className="w-full">
                          {(result.qa_pairs || []).map((qa, index) => (
                            <AccordionItem
                              key={index}
                              value={`item-${index}`}
                              className="border-b-zinc-100 dark:border-zinc-800/50"
                            >
                      ...
                      >
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4">
                          {activeSources.map((source, idx) => (
                            <div
                              key={idx}
                              className="bg-zinc-50 dark:bg-zinc-900/50 rounded-lg border border-zinc-200 dark:border-zinc-800 overflow-hidden"
                            >
